...
| # | 파일 | 한글 제목 | 역할 |
|---|------|-----------|------|
| 01 | `hello-mom.html` | 여보세요 | 소설/공포 — AI 딥보이스 피싱, 47분 만에 3,700만 원 소실 |
| 02 | `not-your-money.html` | 본인의 돈이 아닙니다 | 소설/스릴러 — 경찰/금감원/은행 환급 절차, 돌아온 건 18,240원 |
...
| # | 파일 | 한글 제목 | 역할 |
|---|------|-----------|------|
| 01 | `no-data-yet.html` | 데이터가 없다 | 데이터 — GPU 서버를 사고 가장 먼저 해야 할 일, RAG 먼저 파인튜닝은 나중 |
...
| # | 파일 | 한글 제목 | 역할 |
|---|------|-----------|------|
| 01 | `empty-seat.html` | 빈 자리 | 소설 — 천추궁의 이변, 일곱 번째 기사의 흔적, 여섯 자리가 사라진다는 경고 |
| 02 | `mirror-forest.html` | 거울의 숲 | 소설 — 뒤집힌 차원, 자신의 그림자와 대면, 혜성의 봉인된 기억 |
| 03 | `inverted-tower.html` | 거꾸로 된 탑 | 소설 — (예정) |
...
| # | 파일 | 한글 제목 | 역할 |
|---|------|-----------|------|
| 01 | `euljiro-midnight.html` | 을지로, 자정 | 소설 — 을지로 골목의 이상한 간판, 사라진 사람, 겹쳐진 밤 |
| 02 | `second-night.html` | 두 번째 밤 | 소설 — 서연의 복귀, 야경의 세계, 이면의 방황자, 도영의 시험 |
...
| # | 파일 | 한글 제목 | 역할 |
|---|------|-----------|------|
| 01 | `signing-bonus.html` | 사이닝 보너스 | 소설 — 넥스트비전 퇴사, 토스 NEXT 챌린지, 7,200만 원 + 사이닝 보너스 3,800만 원 |
| 02 | `forty-seven-comments.html` | 코드 리뷰 47건 | 소설 — 첫 PR에 쏟아진 47건의 코드 리뷰, 토스 코드 품질 기준의 벽 |
| 03 | `build-and-break.html` | 만들고 부수고 | 소설 — 스프린트 데모, 피처 플래그, A/B 테스트, 빠르게 만들고 빠르게 버리는 문화 |
...
| # | 파일 | 한글 제목 | 역할 |
|---|------|-----------|------|
| 01 | `first-day.html` | 신입 로봇의 첫 출근 | 소설 — 무인택시 타고 혼자 출근한 K-7, 사번 R-0001, 모니터도 의자도 없는 자리 |
| 02 | `pair-programming.html` | 케이블로 대화하는 법 | 소설 — 0.3초 코드리뷰, 바이브 코딩 논쟁, 네트워크 끊기자 손코딩하는 K-7 |
| 03 | `team-dinner.html` | 회식에 로봇을 데려갈 수 있나요 | 소설 — 삼겹살집 완벽 굽기, 노래방 100점의 공허, 동료란 무엇인가 |
...
| 07 | `882-lines-to-ulsan.html` | 882줄의 울산 | 소설/에필로그 I — 울산 공장의 K-7 재회, 882줄 코드의 73개 주석, 맥락을 전달하는 코드 |
...
## 시리즈 40: AI 채팅 파이프라인 구조 (ai-conversation-pipeline-series) `tech`

| # | 파일 | 한글 제목 | 역할 |
|---|------|-----------|------|
| 01 | `anatomy-of-a-message.html` | AI 채팅 파이프라인 구조 | 전체 파이프라인 구조 — 쉬운 부분과 설계 판단이 필요한 3가지 결정 |

## 시리즈 41: 명세와 하네스 (harness-series) `tech`

> Series 29 「코드는 마지막이다」에서 분리한 단독 시리즈.

| # | 파일 | 한글 제목 | 역할 |
|---|------|-----------|------|
| 01 | `spec-and-guardrails.html` | 명세와 하네스 | 가이드 — 로드맵·RFP·PRD(SDD)·TDD(Red-Green)·하네스(hook 물리적 차단)·settings.json 스코프, 핵심요약·목차·복붙 프롬프트 |
