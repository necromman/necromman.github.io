...
## 7. 후속 기획 — 순천향대 미팅 후 (2026-02-20)

### 배경

순천향대학교 글로컬대학30 사업 미팅(2026-02-20) 후, 고객사(김인성 팀장, 임상현 개발팀) 설득을 위한 후속 콘텐츠 기획. 기존 01편 "바꿔도 됩니다"는 일반론 수준으로, 미팅 전에 보여주려 했으나 못 보여줌.

### 핵심 원칙

- **교육형 콘텐츠** — 읽는 사람이 배우고, 자연스럽게 "이걸 할 수 있는 곳"을 떠올리는 구조
- **프로스트 직접 어필 금지** — 본문에 "우리 회사는~" 식의 영업 멘트 없음. 푸터 크레딧만
- **상급자에게 보여줄 수 있는 콘텐츠** — 김인성 팀장이 처장님께 보여드릴 수 있는 수준
- **간결함** — 주절주절하지 않는다. 시각적 비교와 짧은 문장으로 구성

### 미팅에서 포착한 고객 니즈 → 콘텐츠 대응

| 고객 니즈 | 콘텐츠 대응 |
|-----------|-----------|
| React 전환하고 싶지만 리스크 불안 | 편 02: 실전 개발 프로세스 비교로 구체화 |
| 모바일+PC 단일 소스 희망 | 편 02: React 단일 소스 전략 설명 |
| AI 역량 없음 ("AI 못 만든다") | 편 02: 바이브코딩으로 React+AI 시너지 |
| 학생서비스(비교과) 분리 희망 | 편 03: 행정(Builder6)+학생(React) 분리 아키텍처 |
| 컴포넌트 재사용/유지보수 우려 | 편 03: React 컴포넌트 구조의 유지보수 장점 |
| AI 챗봇/에이전트 관심 | 편 03: 학생서비스 AI 연동 실용 가이드 |
| 전자결재 50% → 완전 자동화 | 편 03: BPM 연계 가능성 |

### 후속 시리즈 구성 (2편 추가)

#### 02. 같은 화면, 다른 세계
- **슬러그**: `different-world.html`
- **역할**: 실전 개발 비교 + 바이브코딩 + 단일 소스
- **태그**: 비교

**Part 구성:**
- Part I — 이 화면을 만든다: 비교과 신청 화면을 예시로 설정
- Part II — 만드는 과정이 이렇게 다르다: WebSquare/eXBuilder vs React 개발 프로세스 비교 (7개 항목)
- Part III — AI가 코드를 쓴다: 바이브코딩 시연 (React prompt → 코드 생성 vs 상용 플랫폼 → 불가)
- Part IV — 소스 하나로 끝낸다: 단일 소스 전략, 모바일 대응

**설득 구조:** 읽는 사람이 "만드는 방법이 이렇게 다르구나"를 이해하고, "AI도 도울 수 있구나"를 알게 됨

#### 03. 행정은 그대로, 학생 서비스만 바꾼다
- **슬러그**: `split-architecture.html`
- **역할**: 분리 아키텍처 + React 구조적 장점 + AI 연동
- **태그**: 아키텍처

**Part 구성:**
- Part I — 왜 전부 바꾸지 않는가: 행정(Builder6) 유지 + 학생서비스(React) 분리의 합리성
- Part II — 분리 아키텍처: API 기반 연계 구조. React의 컴포넌트 재사용/유지보수/테스트 장점
- Part III — 학생 서비스에 AI를 넣으면: RAG 기반 학사 질의, 비교과 추천, 에이전트 오케스트레이션
- Part IV — 전자결재도 연결된다: BPM 연계로 승인 프로세스 자동화

**설득 구조:** 김인성 팀장이 상급자에게 "이 구조로 하면 됩니다"라고 설명할 수 있는 근거 제공

### 시리즈 전체 흐름 (최종)

```
편 01  "바꿔도 됩니다"           ← 배움: React가 왜 괜찮은가 (일반론)
편 02  "같은 화면, 다른 세계"     ← 배움: 실전에서 어떻게 다른가 (개발 비교)
편 03  "행정은 그대로, 학생만"    ← 배움: 우리 상황에 어떻게 적용하는가 (아키텍처)
                                    ↓
                            "이걸 할 수 있는 곳은..."
```
...
- [x] 기획 완료 (2026-02-20)
- [ ] 02편 `different-world.html` 제작
- [ ] 03편 `split-architecture.html` 제작
- [ ] `series-nav.js` 업데이트
