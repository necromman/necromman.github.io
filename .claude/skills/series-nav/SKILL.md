...
2. 새 HTML 파일에 `<script src="../../assets/series-nav.js" defer></script>` 추가
3. HTML에 `.footer` 요소가 있는지 확인 (series-nav는 `.footer` 앞에 삽입된다)
...
```
2. 시리즈 키는 `content/` 아래 폴더명과 일치시킨다
3. `label`은 `Series 02`, `Series 03` 순으로 넘버링한다
